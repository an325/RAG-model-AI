...
def ask_question(query, history):
    global qa
    if qa is None:
        return "⚠️ Please upload a sosreport first!", history
    result = qa.run(query)
...
with gr.Blocks() as demo:
    with gr.Tab("Upload sosreport"):
        sos_file = gr.File(label="Upload sosreport.tar.xz")
        output = gr.Textbox(label="Status Log", lines=20)
        sos_file.upload(process_sosreport, sos_file, output)

    with gr.Tab("Ask Questions"):
        chatbot = gr.Chatbot()
        msg = gr.Textbox(placeholder="Ask about sosreport logs, configs, or system state...")
