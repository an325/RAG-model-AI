...
def process_sosreport(file_path, model_name, progress=gr.Progress()):
    global qa
    status_msgs = []
    ...
    # Step 5: Setup QA chain with selected Ollama model
    status_msgs.append(f"⚙️ Setting up QA chain with Ollama ({model_name})...")
    progress(0.95, desc="Setting up QA chain...")
    llm = OllamaLLM(model=model_name)
    retriever = vectordb.as_retriever(search_kwargs={"k": 5})
    qa = RetrievalQA.from_chain_type(llm=llm, retriever=retriever)

    status_msgs.append("✅ sosreport uploaded and indexed successfully!")
...
def ask_question(question):
    global qa
    if qa is None:
        return "⚠️ Please upload and process a sosreport first."
...
with gr.Blocks() as demo:
    with gr.Row():
        sos_file = gr.File(label="Upload sosreport.tar.xz")
        model_name = gr.Dropdown(choices=["llama3", "mistral", "llama2:7b"], value="mistral", label="Select Model")
    output = gr.Textbox(label="Status Log", lines=20)
    question = gr.Textbox(label="Ask a Question")
    answer = gr.Textbox(label="Answer", lines=10)

    sos_file.upload(process_sosreport, [sos_file, model_name], output)
